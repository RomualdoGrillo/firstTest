...
function ATOMCreateDefinition(startNode){
	...
	//*********************** definendum **********************
	var $definendum=ATOMclone(prototypeSearch())
...
function prototypeSearch2(className,symbolname,dataType,loadedFile){//alcune csassi, ad esempio "ci", possono avere vari datatype 
	//search in dom
...
//crea uno specifico caso dal forall
				if( GetforAllContent($forall).children().length != 0 ){//se For each non vuoto
					...
					$clone.addClass('waiting'); //metti il clone in stato waiting
					attachEventsAndExtend($clone);//dai vita
...
function immediateAssValid(mouseDownNode,op){
