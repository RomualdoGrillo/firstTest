...
function validForDist(mouseDownNode,op,opD){//op2 è il tipo di operazione sulla quale si distribuisce
...
function ATOMdistribute(dragged,target,op,opD){
	var $dragged = $(dragged)// tolto var altrimenti non disponibile dentro each??
...
function validForColl(mouseDownNode,op,opD){
	var $mouseDownNode=$(mouseDownNode);
	var $parent = ATOMparent($mouseDownNode);
...
function ATOMcollect($dragged,$target,op,opD){
	console.log("collect")
	var extOp 
	...
	$(".couldBeCollected").addClass("toBeCollected")
